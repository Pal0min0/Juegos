...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    localStorage.setItem('gamezone_cart', JSON.stringify(cartItems));
  }, [cartItems]);
    ...
    setCartItems(prevItems => {
      ...
      if (existingItem) {
        ...
        if (newQuantity > product.stock) {
          showWarning(`Solo hay ${product.stock} unidades disponibles`);
        ...
        showSuccess(`${product.nombre} actualizado en el carrito`);
        return prevItems.map(item =>
          item.id_producto === product.id_producto
  ...
  const removeFromCart = (productId) => {
    setCartItems(prevItems => {
      ...
      if (item) {
        showSuccess(`${item.nombre} removido del carrito`);
      }
      ...
      return prevItems.map(item => {
        if (item.id_producto === productId) {
          ...
          if (newQuantity > item.stock) {
            showWarning(`Solo hay ${item.stock} unidades disponibles`);
